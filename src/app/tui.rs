...
const CATEGORIES: [&str; 7] = ["All", "Browser", "IDE", "Runtime", "Dev Tool", "Utility", "Other"];
...
impl AppTui {
    ...
    fn event_loop(&mut self, terminal: &mut Terminal<CrosstermBackend<Stdout>>) -> Result<Vec<AppEntry>> {
        loop {
            ...
            if event::poll(Duration::from_millis(200))? {
                if let Event::Key(key) = event::read()? {
                    ...
                    match key.code {
                        ...
                        KeyCode::Enter => return Ok(self.apps.iter().filter(|a| a.selected).cloned().collect()),
                        _ => {}
                    }
                }
            }
    ...
    fn render(&mut self, f: &mut ratatui::Frame) {
        ...
        let status = Paragraph::new(vec![
            Line::from(vec![
                ...
                Span::raw(if self.search.is_empty() { "<none>" } else { &self.search }),
                Span::raw("    "),
                Span::styled("Selected only: ", Style::default().fg(Color::Yellow)),
                Span::raw(if self.selected_only { "on" } else { "off" }),
            ])
        ]).block(Block::default().borders(Borders::ALL).title(if self.search_mode { "Search mode" } else { "Status" }));
        ...
        let items: Vec<ListItem> = visible
            ...
            .map(|&idx| {
                ...
                ListItem::new(format!("{} [{}] {} v{}", mark, category, app.name, app.version))
            })
            .collect();
    ...
    fn visible_indices(&self) -> Vec<usize> {
        self.apps
            ...
            .enumerate()
            .filter(|(_, app)| {
                let category_ok = self.category_idx == 0 || app::category_for_app(app) == CATEGORIES[self.category_idx];
                let search_ok = self.search.is_empty()
                    || app.name.to_lowercase().contains(&self.search.to_lowercase())
    ...
    fn toggle_current(&mut self) {
        ...
        let Some(&idx) = visible.get(sel) else { return; };
